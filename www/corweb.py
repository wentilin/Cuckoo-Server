...
class RequestHandler(object):
    ...
    def __call__(self, request):
        ...
        try:
            if hasattr(self._func, '__auth__') and self._func.__auth__:
                yield from auth(request)
            r = yield from self._func(**kw)
            return r
        except APIError as e:
            traceback.print_exc()
            return dict(code=e.code, data=e.data, msg=e.msg)
        except Exception as innerEx:
            traceback.print_exc()
            logging.error('api error(%s):%s' % (self._func.__name__, innerEx))
            return dict(code=500, data=innerEx.__cause__, msg='服务器内部发生错误')
...
def add_routes(app, module_name):
    ...
    for attr in dir(mod):
        ...
        if callable(fn):
            method = getattr(fn, '__method__', None)
            path = getattr(fn, '__route__', None)
            auth = getattr(fn, '__auth__', False)
            if method and path:
                add_route(app, fn, auth)
