            ...
            if isinstance(v, Field):
                ...
                if v.primary_key:
                    ...
                    if hasattr(v, 'auto_increment') and v.auto_increment:
                        primaryKeyIsIncrement = True
                    primaryKey = k
                else:
                    if not hasattr(v, 'auto_increment') or not v.auto_increment:
                        fields.append(k)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def update(self):
        ...
        if rows != 1:
            logging.warning('failed to update by primary key(%s) affected rows: %s' % (primary_key, rows))
